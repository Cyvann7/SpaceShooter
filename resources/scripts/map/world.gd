...
func make_map():
	...
	for cell in Map.get_used_cells_by_id(0):
		...
		if (randi()%1000)/10 < 0.5: 
			var e = Enemy.instance()
			$Enemy.add_child(e)
			e.position = Vector2(cell.x*tile_size,cell.y*tile_size)
	
	var amt_of_flr_tiles = Map.get_used_cells_by_id(0).size()
	var spawn_cell = Map.get_used_cells_by_id(0)[randi()%amt_of_flr_tiles]
	start_room = Map.map_to_world(spawn_cell)
	
	
	#get rid of cringe squares
